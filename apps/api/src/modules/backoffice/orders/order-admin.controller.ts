...
export async function approveOrder(req: Request, res: Response) {
  const { orderId } = orderIdParamSchema.parse(req.params);
  const result = await orderService.approveOrder(orderId, req.user!.userId);
...
export async function rejectOrder(req: Request, res: Response) {
  ...
  const result = await orderService.rejectOrder(orderId, req.user!.userId, reason);
...
export async function advanceStatus(req: Request, res: Response) {
  ...
  const result = await orderService.advanceOrderStatus(orderId, status);
