...
export async function createStaff(req: Request, res: Response) {
  ...
  const user = await userAdminService.createPrivilegedUser(body, 'STAFF');
  sendSuccess({ res, message: 'Staff user created', data: user, statusCode: 201 });
}

export async function createAdmin(req: Request, res: Response) {
  const body = createPrivilegedUserSchema.parse(req.body);
  const user = await userAdminService.createPrivilegedUser(body, 'ADMIN');
...
export async function updateUser(req: Request, res: Response) {
  ...
  const user = await userAdminService.updateUser(userId, body);
  sendSuccess({ res, message: 'User updated', data: user });
}

export async function disableUser(req: Request, res: Response) {
  const { userId } = userIdParamSchema.parse(req.params);
  const user = await userAdminService.disableUser(userId, req.user!.userId);
